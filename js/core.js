...
/**
 * Blink function. Basic usage (Most common settings. {no max blinks = blink indefinitely, blinkDuration ~1 sec, no callback}):
...
//Shorten function converts a text into a ...READ MORE text depending on the chars count
...
;(function($){
	$.fn.typewriter = function( speed, callback, time_before ) {
		...
		var write = function( e, text, time ) {
			//We add support for inactive tabs in Chrome. It is an issue of design of chrome itself. here:			//http://stackoverflow.com/questions/5927284/how-can-i-make-setinterval-also-work-when-a-tab-is-inactive-in-chrome?s=2b3e7c34-9910-4053-9e82-97dec0b55b17#new-answer
...
/*
 * Returns the time in millisecons since Unix epoch
 */
function getTime(){
	var d = new Date();	
	return d.getTime();
...
/*
 * Convert to js Date object from unixtimestamp
 */
	...
	if ($('#comment_textarea').val().length > 0){
		ajax_request(
			...
			function(){ 
				...
				$("#comment_status").css('visibility', 'visible');
				$("#comment_status").html(ssLang.site.commentSentChat);
				$("#comment_status").blink({maxBlinks: 2, blinkPeriod: 200}); 
				setTimeout(function(){ $("#comment_status").css('visibility', 'hidden');},3000); 
				//$("#comment_status").fadeTo("slow", 0.0);
			},
			function(){ },
			function(){$("#comment_status").css('visibility', 'visible'); $("#comment_status").html(ssLang.site.errorSendingComment); },
			"POST"
		);
	}
}
...
function reset_comments(){
	reset_comment=true;
	$("#comments_board").html(ssLang.site.waitingForComments);
	comment_id = 0;	
}
...
function load_comments(comments){
	if(pending_comment == true){
		...
		for(var i in comments) {	
			...
			new_line += '<div class="left_comment_side">';
			new_line += '<a href="'+ ssBaseUrl + '/profile/'+comments[i]['username']+'"><img class="user_image_icon" style="vertical-align:middle" src="'+ comments[i]['user_image'] + '" width="45" height="45"></a>';
			new_line += '</div>';
			new_line += '<div class="right_comment_side">';
			new_line += '<div class="comment_header">';
			display_name = (comments[i]['user_firstname']) ? comments[i]['user_firstname'] : '';
			display_name += (comments[i]['user_lastname']) ? ' '+ comments[i]['user_lastname'] : comments[i]['username'];
			new_line += '<span class="comment_username"><a href="'+ssBaseUrl+'/mysub/'+ comments[i]['username'] +'">'+ display_name.toUpperCase() + '</a></span>';
			...
			new_line += '<span class="comment_vote"> <a class="comment_like_button" alt="'+comments[i]['comment_id']+'"></a> <em class="comment_votes_count total_likes total_likes'+ comments[i]['likes']+'">'+ comments[i]['likes']+'</em> <a class="comment_dislike_button"  alt="'+comments[i]['comment_id']+'"></a> <em class="comment_votes_count total_dislikes total_dislikes'+comments[i]['dislikes']+'">'+comments[i]['dislikes']+'</em></span>';
			new_line += '</div>';
			new_line += '<div class="comment_text">'+ comments[i]['comment_text']+'</div>';
			new_line += '</div>';
			
			new_line += '</div>';
			new_line += '<div class="clear_both"></div>';
			new_line += '<hr class="page_hrline" style="margin:20px 0px 20px 0px;">';			
			$("#comments_board").prepend(new_line);
			$("#c"+comments[i]['comment_id']+" .comment_text").shorten({"showChars" : 260});			
...
function get_Contents(callback){
	...
	ajax_request(
		...
		function(json){			
			...
			if(time_remaining != json.time_remaining && json.time_remaining > 10){//We should not change the value if is not greater than 10
			...
			var aa = setTimeout("get_Contents()",(ssRequestInterval*1000));//(add the javascript milliseconds) Everythig loaded ok, lets make a new request to watch for new changes			
		},
		function(){
			if(callback != undefined) callback();
			setTimeout(function(){
				$("#header_error").text(ssLang.site.errorGettingData);
				$("#header_error").show();
				}
			,5000);			
			var ba = setTimeout(function(){$("#header_error").hide()},10000);
			var bb = setTimeout("get_Contents()",(ssRequestInterval*1000) + 5000);//There was an error loading content, lets make a new request to try to get content again
		}
	);
}
...
$('.comment_like_button, .comment_dislike_button').die('click').live('click',function() {
	var link = ssBaseUrl+'/api/v1/comment/vote?comment_id='+$(this).attr('alt')+'&vote=';			
...
function subject_voting(){
//Use die() to remove any previous attached click event, and use live() to preserve the event if the DOM changes
$('.sub_like_button, .sub_dislike_button').die('click').live('click',function() {
	var link = ssBaseUrl+'/api/v1/subject/vote?subject_id='+$(this).attr('alt')+'&vote=';			
...
function countdown(){
	if(time_remaining >=0){//when to start the time_remaining
		...
		var minutes = hms.m;
		var seconds = hms.s;
		//We need to conditional pad, on IE7 won't work
		if(hms.m < 10) minutes = ("0" + hms.m).substr(-2);//pad with 0
		if(hms.s < 10) seconds = ("0" + hms.s).substr(-2);//pad with 0
		$("#comment_timer").html( minutes + ':' +  seconds + '');
		if(time_remaining > 60){
			...
			if(time_remaining < 30){
				if(time_remaining > 25){
					...
					$("#comments_title").html(ssLang.site.commentsClosing);
					$("#comments_title").fadeTo("medium", 0.0);
				}else if(time_remaining > 15){
					$("#comments_title").attr("style", "");					
				}else if(time_remaining == 10){
					$("#comment_timer").blink({maxBlinks: 10, blinkPeriod: 1000}); 					
					...
					$("#comments_title").html(ssLang.site.commentsClosed);
					$("#comment_textarea").attr('disabled',true);																							
				}else{
					if(time_remaining == 6){						
						$("#header_title h1").attr("style", "color:#1C75CE");
						$("#header_title h1").html(ssLang.site.changingToNextSubject);
...
function display_elements(obj_json){
	...
	display_name = (obj_json.user_firstname) ? obj_json.user_firstname : '';
	display_name += (obj_json.user_lastname) ? ' '+ obj_json.user_lastname : obj_json.username;	
	submit_info = '<a href="'+ssBaseUrl+'/mysub/'+obj_json.username+'">' + '<img class="user_image_icon" style="vertical-align:middle" src="'+ obj_json.user_image + '" width="25" height="25">' + '</a> ';
	submit_info += '<a href="'+ssBaseUrl+'/mysub/'+obj_json.username+'">' + display_name + '</a>';
	submit_info += '<br />' + time_submitted.getFullYear() +'-'+ time_submitted.getMonth() +'-' +time_submitted.getDate() +' '+time_submitted.getHours()+':'+time_submitted.getMinutes()+' UTC';
	share_links = '<a target="_blank" href="'+ssAddThisLinkUrl+'/forward/facebook/offer?url='+ssBaseUrl+'/sub/'+obj_json.urn+'&title='+obj_json.title+'&pubid=" rel="nofollow"><img src="'+ssAddThisImgUrl+'/facebook.gif"></a>&nbsp;<a target="_blank" href="'+ssAddThisLinkUrl+'/forward/twitter/offer?url='+ssBaseUrl+'/sub/'+obj_json.urn+'&title='+obj_json.title+'&pubid=" rel="nofollow"><img src="'+ssAddThisImgUrl+'/twitter.gif"></a>&nbsp;<a target="_blank" href="'+ssAddThisLinkUrl+'/email/offer?url='+ssBaseUrl+'/sub/'+obj_json.urn+'&title='+obj_json.title+'&pubid=" rel="nofollow"><img src="'+ssAddThisImgUrl+'/email.gif"></a>&nbsp;<a target="_blank" href="'+ssAddThisLinkUrl+'/offer?url='+ssBaseUrl+'/sub/'+obj_json.urn+'&title='+obj_json.title+'&pubid=" rel="nofollow"><img src="'+ssAddThisImgUrl+'/menu.gif"></a><br>';
	$('#subject_voting').html( '<div style="margin-left:10px;float:right;"><div style="float:right;"><span class=""> <a class="sub_like_button" alt="'+obj_json.subject_id+'"></a> <em id="sub_likes_count" class="sub_vote_label" style="margin-right:10px;">'+obj_json.likes+'</em> <a class="sub_dislike_button"  alt="'+obj_json.subject_id+'"></a> <em id="sub_dislikes_count" class="sub_vote_label">'+obj_json.dislikes+'</em></span></div>');
	...
	$('#left_content_div').empty();//some times iframes or object elements stay inside, so clean it just in those rare cases
	if(obj_json.content_type == "video")
		$('#left_content_container').css("border", "1px solid #212121");
	else
		$('#left_content_container').css("border", "1px solid #D9D9D9");
	$('#left_content_div').html(''+obj_json.content_html);
	$('#right_content_div').empty();
	$('#right_content_div').html(''+'<br>'+obj_json.user_comment+'<br>');
	current_info = obj_json.country_name;
	//$("#comments_board").html("Waiting for comments");
	
	$("#comments_title").attr("style", "");
	$("#comments_title").html('Latest user comments:');
	$("#comment_timer").attr("style", "");
	$("#comment_timer").html('00:00');
				
	$("#header_title h1").html(current_title.toUpperCase());
	$("#country_info").html(current_info);
...
$(document).ready(function() {
	//Set-up some behaviours
	//Setup comments
	$('#comment_textarea').keypress(function(event) {//Cause enter keypress submit the text on the text area
		if (event.keyCode == '13') {
			event.preventDefault();
			send_comment();
		}
	});
	$("#comment_textarea").val(ssLang.site.writeYourComments);
